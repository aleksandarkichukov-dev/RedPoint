...
- **A `#` in an `.env` value truncates it, silently.** `SPEEDY_PASSWORD=Paco#1202` reaches the process as `Paco`, because `#` starts a comment. The file looks right, every check on the file passes, and the API answers "invalid credentials" — which sends you to the provider instead of to the file. Quote any value that could contain one: `SPEEDY_PASSWORD="Paco#1202"`. Cost an evening on 6 August 2026.
- **Speedy answer in capitals.** Settlements come back as `ВАРНА`, not `Варна`, so anything matching their data by name has to fold case or it silently finds nothing.
...
- **Reading an order's totals needs `shipping_methods.*`.** They are computed, and computing them resolves the shipping method's adjustments, which needs its `version`. Ask for `shipping_methods.name` and the query either throws "Shipping method version is required to load adjustments" or returns every total as `undefined` depending on what else is selected. `items.total` needs `items.*` for the same reason. Silent when it fails — the confirmation email rendered "0,00 €" and "x undefined".
  ...
  **Listing many orders needs the order's own `version` as well.** Filtering to a single order gets away without it; a list query with `total` throws the same "Shipping method version is required" even with both wildcards present. `src/lib/unpaid-orders.ts` has the full working field list.
